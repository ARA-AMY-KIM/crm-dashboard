...
> 최초 배포: 2026년 6월  
> 최종 업데이트: 2026년 7월 9일 — 상태 필터 개편(기획/작업 진행 분리), 상태 배지 색상 개편, 정렬 수정 + 정렬 시 Plan 자동 숨김, 완료·보류 숨기기 토글, 월 필터 기본값=현재 월  
> 담당: 김아라  
...
### 상태 필터 칩 (2026-07 개편) — `passFilter`
칩: `전체 / 할 일 / 기획 진행 중 / 작업 진행 중 / 배포 대기 / 완료` (단일 선택)

| 칩 (F.st 값) | 매칭 |
|---|---|
| 할 일 (`todo`) | isTodo — 할 일, # 할 일, 이슈 오픈, Backlog/백로그 |
| 기획 진행 중 (`planip`) | isPlanningIP — `# 기획 진행 중` 만 |
| 작업 진행 중 (`workip`) | isInProg && !isPlanningIP — 기획 완료, 디자인 대기/진행/검토, 디자인 작업 완료, 개발 대기/진행, QA 대기/진행, 작업 진행 중, 진행 중 |
| 배포 대기 (`deploy`) | isDeploy — # 배포 대기, 배포 대기 |
| 완료 (`done`) | isDone — 최종 완료, # 최종 완료 |

> 보류·이슈 아님(isNoIssue)은 전용 칩이 없다. `전체`에서만 표시되고 `완료·보류 숨기기` 토글로 숨긴다.
> `기획 진행 중` vs `작업 진행 중`은 `isPlanningIP(s)`(= `# 기획 진행 중` / `기획 진행 중`)로만 갈린다. 나머지 진행 상태는 전부 작업 진행 중.

### 카운트 요약 (상단)
`완료 / 기획진행 / 작업진행 / 배포대기 / 할일` — 칩과 동일 기준. 기획진행 = isPlanningIP, 작업진행 = 나머지 isInProg. **보류/이슈 아님은 카운트에 표시하지 않음**(전체 건수 tcnt와 합이 다를 수 있음).

### 상태 배지 색상 (ST_MAP, 표시 전용 — 필터 분류와 별개)
| 색 | 상태값 |
|---|---|
| 초록 솔리드 `#fff`/`#16a34a` (강조) | 최종 완료, # 최종 완료 |
| 핑크 솔리드 `#fff`/`#db2777` (강조) | # 기획 진행 중 |
| 시안 `#0891b2`/`#cffafe` | # 개발 진행 중, # 디자인 진행 중, # QA 진행 중, # QA 대기, # 작업 진행 중, 진행 중 |
| 인디고 `#4f46e5`/`#e0e7ff` | 기획 완료, 디자인 작업 완료, # 디자인 대기, # 개발 대기, # 디자인 검토 대기 |
| 노랑 `#d97706`/`#fef3c7` | # 배포 대기, 배포 대기 |
| 회색 `#94a3b8`/`#f1f5f9` | 할 일, # 할 일, 이슈 오픈, Backlog/백로그 |
| 중간 회색 `#6b7280`/`#f3f4f6` | 이슈 아님, # 이슈 아님, 보류, # 보류 |

> ⚠️ 배지 색(ST_MAP)과 필터 분류(passFilter)는 **별개**다. 예: '작업 진행 중' 필터 한 칸에 시안·인디고 배지가 섞여 있다. 배지 색만 바꿀 땐 ST_MAP, 필터 분류를 바꿀 땐 passFilter/Set을 건드릴 것.
...
### 정렬
- 기한/완료일 · 업데이트일 헤더 클릭으로 정렬 (**작업 티켓 기준**)
- 클릭 순서: ↓ 최신순 → ↑ 오래된순 → ↕ 정렬 없음 (초기화)
- 기본값: 정렬 없음 (Jira API 응답 순서)
- 기한/완료일 정렬 값은 **화면 표시값과 일치** — 완료 상태면 resolutiondate, 아니면 duedate (`getVal`).
- 정렬 화살표/하이라이트는 `render()`가 sortKey/sortDir 기준으로 그림(setSort는 상태만 변경). 예전엔 render가 헤더를 다시 그려 화살표가 리셋되던 버그가 있었음.
- **정렬 활성 시 Plan 자동 숨김**: sortKey가 있으면 Plan 하위 행이 숨겨지고 'Plan 같이 보기' 토글이 비활성화됨(정렬 해제 시 복원). Plan이 자기 날짜를 달고 섞여 보이던 문제 방지 (`_sortActive`).
...
### 완료·보류 숨기기 토글 (2026-07-09 추가)
- 테이블 헤더 바에 토글 스위치 (`완료·보류 숨기기`), Plan 토글 옆
- 켜짐: **task 기준** 상태가 `최종 완료`(isDone) 또는 `이슈 아님/보류`(isNoIssue)인 작업 티켓을 목록에서 제외 → 진행 중인 작업만 표시. 숨겨진 task의 하위 Plan도 세트로 함께 제외됨.
- 켜짐일 때 상태 필터의 `완료` 칩은 비활성화(disabled)되며, `완료`가 선택돼 있었으면 `전체`로 자동 복귀. (이슈 아님은 전용 칩이 없음.) (`updateStatusChipsDisabled()`, `toggleHideInactive()`)
- 기본값 OFF. passFilter에서 `if(hideInactive && (isDone||isNoIssue)) return false`로 처리.

### 설정 저장 (localStorage)
브라우저별 저장, 다음 방문에도 유지:
- `chapter_favs` — 즐겨찾기 티켓 키 목록
- `chapter_showPlan` — Plan 같이 보기 토글 (기본 true)
- `chapter_hideInactive` — 완료·보류 숨기기 토글 (기본 false)

### 데이터 로드 실패 처리
Jira API 조회 실패 시 목업/샘플 데이터를 보여주지 않고, 테이블 영역에 "데이터를 불러오지 못했습니다" 안내만 표시한다(목업 코드 없음). 상태 표시줄은 '로드 실패'.

### URL 파라미터 — `applyUrlParams()`
담당자 또는 월 고정 링크 생성 가능. `who`는 담당자 명단(현재 6인)에 포함된 값만 적용된다.
